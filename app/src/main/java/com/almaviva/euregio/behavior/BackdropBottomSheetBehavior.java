package com.almaviva.euregio.behavior;
 ...
 * {@link BottomSheetBehavior} in the XML file to get same behavior like Google Maps.
...
public class BackdropBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    /**
     * To avoid using multiple "peekheight=" in XML and looking flexibility allowing {@link BottomSheetBehavior#mPeekHeight}
     * get changed dynamically we get the {@link NestedScrollView} that has
     * "app:layout_behavior=" {@link BottomSheetBehavior} inside the {@link CoordinatorLayout}
     */
    ...
    /**
     * Look into the CoordiantorLayout for the {@link BottomSheetBehavior}
     * @param coordinatorLayout with app:layout_behavior= {@link BottomSheetBehavior}
     */
    private void getBottomSheetBehavior(@NonNull CoordinatorLayout coordinatorLayout) {
        ...
        for (int i = 0; i < coordinatorLayout.getChildCount(); i++) {
            ...
            if (child instanceof NestedScrollView) {
                ...
                try {
                    BottomSheetBehavior temp = BottomSheetBehavior.from(child);
